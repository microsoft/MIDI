...
class Build : NukeBuild
{
    ...
    MidiBuildType BuildType = MidiBuildType.RC;  // 从 SDK_BuildNumber.txt 读取，默认为 RC
    
    UInt16 BuildVersionMajor;             // 从 SDK_BuildNumber.txt 读取
    UInt16 BuildVersionMinor;             // 从 SDK_BuildNumber.txt 读取
    UInt16 BuildVersionPatch;             // 从 SDK_BuildNumber.txt 读取
    UInt16 BuildVersionPreviewNumber;     // 从 SDK_BuildNumber.txt 读取
    UInt16 BuildVersionPreviewMinorNumber;      // 从 SDK_BuildNumber.txt 读取
    UInt16 BuildVersionPreviewBuildNumber;       // 从 SDK_BuildNumber.txt 读取
    bool BuildVersionPreviewHasMinorNumber; // 用户是否设置了 Minor 版本号（区分 3.0 和 3.0.0 格式）
    string VersionName => BuildType == MidiBuildType.Stable 
        ? "SDK " + BuildMajorMinorPatch 
        : BuildType == MidiBuildType.RC 
            ? "SDK Release Candidate " + BuildVersionPreviewNumber 
            : "SDK Preview " + BuildVersionPreviewNumber;
   ...
   // AbsolutePath MidiSettingsSetupSolutionFolder => UserToolsRootFolder / "midi-settings-setup";
    ...
    Target T_Prerequisites => _ => _
        .Executes(() =>
        {
            ...
            {
                var rootPath = NukeBuild.RootDirectory.ToString();
                // Ensure the path ends with a backslash for proper concatenation in WiX files
                if (!rootPath.EndsWith("\\"))
                {
                    rootPath += "\\";
                }
                Environment.SetEnvironmentVariable("MIDI_REPO_ROOT", rootPath);
            }
            ...
            {
                // 兼容两种格式：
                // - 3.0 格式 -> rc.3.0 (用户没有设置 Minor)
                // - 3.0.0 格式 -> rc.3.0.0 (用户设置了 Minor，即使为 0 也要显示)
            ...
            {
                // 兼容两种格式：
                // - 3.0 格式 -> preview.3.0 (用户没有设置 Minor)
                // - 3.0.0 格式 -> preview.3.0.0 (用户设置了 Minor，即使为 0 也要显示)
                if (BuildVersionPreviewHasMinorNumber)
                {
    ...
    Target T_BuildAndPackAllAppSDKs => _ => _
        ...
        //    bool wxsWritten = false;
            ...
            // Restore NuGet packages for the solution before building
            Console.Out.WriteLine($"----------------------------------------------------------------------");
            Console.Out.WriteLine($"Restoring NuGet packages for App SDK solution...");
    ...
    Target T_BuildAppSdkRuntimeAndToolsInstaller => _ => _
        ...
        .Executes(() =>
        {
            ...
            foreach (var platform in InstallerPlatforms)
            {
                ...
                // First restore the WiX project to ensure project.assets.json is created
                MSBuildTasks.MSBuild(_ => _
                    .SetProcessToolPath(MSBuildTasks.MSBuildPath)
    ...
    // 默认版本配置内容
    const string DefaultVersionFileContent = @"# Windows MIDI Services SDK Version Configuration File
# Windows MIDI Services SDK 版本配置文件
...
# Line 3: Preview.Minor.Build (e.g.: 3.0.0 or 3.0) [Optional]
# 第3行：预览版本号.次版本号.构建号（例如：3.0.0 或 3.0）[可选]
#
# Note:
# 注意：
...
#
# Example:
# 示例：rc 1.0.15 3.0.0 -> 1.0.15-rc.3.0.0
...
";
    ...
    void IncrementBuildNumber()
    {
        ...
        // 第3行：预览版本号.次版本号.构建号 (可选，支持 3.0.0 或 3.0 格式，默认为 3.0.0)
        // 以 # 开头的行被视为注释，会被忽略
            ...
            if (File.Exists(BuildVersionFile))
            {
                ...
                if (validLines.Length >= 2)
                {
                    ...
                    if (validLines.Length >= 3)
                    {
                        ...
                        // 读取次版本号和构建号
                        // 支持 Preview.Minor.Build (3.0.0) 或 Preview.Build (3.0) 格式
                        if (previewBuildParts.Length >= 3)
                        {
                            // 3.0.0 格式: Preview=3, Minor=0, Build=0
                        ...
                        {
                            // 3.0 格式: Preview=3, Build=0 (不设置 Minor)
    ...
    void UpdateSetupBundleInfoIncludeFile(string platform)
    {
        ...
        using (StreamWriter writer = System.IO.File.CreateText(SetupBundleInfoIncludeFile))
        {
            ...
            // MSI package versions must be in format major.minor.build (e.g., 1.0.15) without pre-release labels
            writer.WriteLine($"  <?define MidiSdkAndToolsVersion=\"{BuildMajorMinorPatch}\" ?>");
            writer.WriteLine("</Include>");
        }
    }
