...
.EXAMPLE
    .\build.ps1 -Target Sdk,Stage -Platform x64
...
param(
    ...
    # The PowerShell projection still targets the pre-rename SDK namespaces and does not
    # compile against the current SDK. Opt in only once it has been ported.
    [switch] $IncludePowerShellModule,
...
$PowerShellProject = Join-Path $SourceRoot 'app-sdk\projections\powershell\WindowsMidiServices.csproj'
...
function Get-BuildVersion {
    ...
    # SemVer 2 for NuGet. Stable drops the prerelease tag entirely.
    if ($json.channel -eq 'stable') {
        $semVer = $majorMinorPatch
    }
    else {
        $semVer = '{0}-{1}.{2}.{3}' -f $majorMinorPatch, $json.channel, $json.channelNumber, $effectiveBuild
    }
...
function Invoke-VersionTarget {
    ...
    # --- BundleInfo.wxi, consumed by every .wxs in the installer ---------------------------
    $bundleInfo = @"
<?xml version="1.0" encoding="utf-8"?>
<!-- Generated by build\build.ps1 from build\version.json. Do not edit. -->
<Include>
...
function Invoke-StageTarget {
    ...
    foreach ($plat in $Platform) {
        ...
        # Each GUI tool gets its own folder AND its own copy of the runtime - activation is
        # process-local, so the SDK has to sit beside the exe.
        foreach ($tool in $GuiTools) {
            $toolStaging = Join-Path $appSdkStaging $tool.Folder
            Copy-Staged -Source (Join-Path $SdkOutRoot "$($tool.Name)\$plat\$Configuration\$($tool.Name).exe") -Destination $toolStaging
        ...
        if ($IncludePowerShellModule) {
            ...
            Copy-Staged -Source (Join-Path (Split-Path -Parent $PowerShellProject) 'WindowsMidiServices.psd1') `
                -Destination $psStaging
        }
    }
...
function New-WixFileListFragment {
    ...
    [void]$sb.AppendLine('<!-- Generated by build\build.ps1. Do not edit; your changes will be overwritten. -->')
...
function New-SetupFileLists {
    ...
    foreach ($tool in $GuiTools) {
        $sdkDirs['app-sdk\$(var.Platform)\' + $tool.Folder] = $tool.DirectoryId
...
function Invoke-SetupTarget {
    ...
    $script:BuiltInstallers = @()

    foreach ($plat in $Platform) {
        Invoke-MSBuild -ProjectOrSolution $SetupSolution -BuildPlatform $plat `
            -Targets @('Restore', 'Rebuild') `
            -Properties @{
                'IncludePowerShell' = [int][bool]$IncludePowerShellModule
            }

        $bundle = Join-Path $SetupSolutionRoot "main-bundle\bin\$plat\$Configuration\WindowsMidiServicesSdkRuntimeSetup.exe"
        if (Test-Path $bundle) {
            $script:BuiltInstallers += [pscustomobject]@{ Platform = $plat; Path = $bundle }
...
function Invoke-ReleaseTarget {
    ...
    if (Get-Variable -Name BuiltInstallers -Scope Script -ErrorAction SilentlyContinue) {
        foreach ($installer in $script:BuiltInstallers) {
            $name = "Windows MIDI Services (SDK Runtime and Tools) $($Version.SemVer)-$($installer.Platform.ToLowerInvariant()).exe"
            Copy-Item $installer.Path -Destination (Join-Path $folder $name) -Force
