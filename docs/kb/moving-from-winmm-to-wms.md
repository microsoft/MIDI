...
## Quick reference: what you used to call, and what to call now

If you have a working WinMM codebase, start here. Each row links to the reference page for the type, and to a runnable sample where one exists. The rest of this article explains the concepts behind the right-hand column.
...
| Get the USB VID and PID | `DRV_QUERYDEVICEINTERFACE`, then parse the interface string | `MidiParentDeviceInformation.UsbVendorId` and `UsbProductId` | [MidiParentDeviceInformation]({{ site.baseurl }}/sdk-reference/Enumeration/MidiParentDeviceInformation/) | [get-vid-pid](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/get-vid-pid) |
| Tell what kind of device this is | `wTechnology` in the device caps, which said very little | `GetTransportSuppliedInfo().TransportCode`, matched against `MidiReporting.GetInstalledTransportPlugins` | [MidiReporting]({{ site.baseurl }}/sdk-reference/Reporting/MidiReporting/) | [identify-endpoint-type](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/identify-endpoint-type) |
| Open a port | `midiInOpen` and `midiOutOpen`, one handle per direction | `MidiSession.Create`, then `CreateEndpointConnection` and `Open`. One bidirectional connection per endpoint | [MidiSession]({{ site.baseurl }}/sdk-reference/MidiSession/), [MidiEndpointConnection]({{ site.baseurl }}/sdk-reference/MidiEndpointConnection/) | [basics](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/basics) |
| Receive from one port only | One handle per port, so this was implicit | `MidiGroupEndpointListener`, because one connection carries up to 16 groups | [MidiGroupEndpointListener]({{ site.baseurl }}/sdk-reference/ClientPlugins/MidiGroupEndpointListener/) | [endpoint-listeners](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/endpoint-listeners) |
| Send one short message | `midiOutShortMsg` | `SendSingleMessageWords`, or `SendSingleMessagePacket` for a strongly-typed message | [MidiEndpointConnection]({{ site.baseurl }}/sdk-reference/MidiEndpointConnection/) | [basics](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/basics) |
| Send several messages at once | One `midiOutShortMsg` call each | `SendMultipleMessagesWordList` and the other multiple-message senders | [MidiEndpointConnection]({{ site.baseurl }}/sdk-reference/MidiEndpointConnection/) | [send-speed](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/send-speed) |
| Send a message at a future time | Not available. You ran your own timer and sent when it fired | Pass a future timestamp to any send function. Build it with `MidiClock.OffsetTimestampByMilliseconds` | [MidiClock]({{ site.baseurl }}/sdk-reference/MidiClock/) | [scheduled-send-messages](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/scheduled-send-messages) |
| Schedule without per-message allocation | Not available | `SendMidiMessagesRaw` takes the same timestamp | [COM Extensions]({{ site.baseurl }}/sdk-reference/MidiEndpointConnection_COM-Extensions/) | [scheduled-messages-com-extensions](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/scheduled-messages-com-extensions) |
| Send a `.syx` file to a device | `midiOutPrepareHeader`, `midiOutLongMsg`, `midiOutUnprepareHeader`, and careful buffer lifetime | `MidiSystemExclusiveSender.SendBinarySysEx7ByteDataAsync` | [MidiSystemExclusiveSender]({{ site.baseurl }}/sdk-reference/Utilities/SysExTransfer/MidiSystemExclusiveSender/) | [sysex-file-sender](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/sysex-file-sender) |
| Receive SysEx and save it | `MIM_LONGDATA`, plus preparing, requeueing and reassembling buffers | `MidiSystemExclusiveReceiver`, which reassembles for you | [MidiSystemExclusiveReceiver]({{ site.baseurl }}/sdk-reference/Utilities/SysExTransfer/MidiSystemExclusiveReceiver/) | [sysex-file-receiver](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/sysex-file-receiver) |
| Send MIDI 1.0 bytes you already have in memory | `midiOutShortMsg` or `midiOutLongMsg`, depending on length | `MidiMessageConverter.ConvertMidi1CompleteMessageBytesToUmpWords`, with a `MidiBytestreamToUmpMessageConverterState` | [MidiMessageConverter]({{ site.baseurl }}/sdk-reference/Utilities/Messages/MidiMessageConverter/) | [sysex-send-bytes](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/sysex-send-bytes) |
| Know how much you can send in one call | Fixed assumptions in your own code | `GetSupportedMaxMidiWordsPerTransmission`, per connection. Never hard-code it | [MidiEndpointConnection]({{ site.baseurl }}/sdk-reference/MidiEndpointConnection/) | [com-extensions](https://github.com/microsoft/MIDI/tree/main/samples/cpp-winrt/com-extensions) |

Two rows deserve a warning rather than a link. **Timestamps** are absolute values on the `MidiClock.Now` timeline counted from system boot, not durations and not relative to when you opened anything, so a WinMM habit of treating a timestamp as a delay will send everything immediately. **Names** are user-editable display metadata now, so the endpoint device id is what identifies a device across runs, not the name.
