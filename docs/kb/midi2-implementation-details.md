...
Specifications leave room for interpretation, however hard everyone tries. The MIDI Association and its members aim to be precise, and most of the gaps get worked out between the operating system companies inside the association. Some, though, are places where an approach simply doesn't make sense on one OS. Here are the Windows-specific ones you should know about as a developer.
...
When we create MIDI 1.0-compatible ports for MIDI 2.0 endpoints, we use the group numbers and the endpoint name. We also have built-in naming algorithms that work differently for native MIDI 1.0 devices, covering both WinMM-compatible naming and a more modern style that uses the iJack and pin values. WinMM port names are limited to 31 characters.
...
2. The name supplied by the transport plugin in the service
