...
| -------- | ----------- |
| `ConnectionId` | The generated GUID which uniquely identifies this connection instance. This is what is provided to the `MidiSession` when disconnecting an endpoint |
...
There are multiple mechanisms available for sending and receiving messages, each suitable to different programming languages and app data storage models.
...
| `SendSingleMessageWordArray (timestamp, startIndex, wordCount, words)` | Send an array of words for a single message. Note: Some projections will send the entire array as a copy, so this may not be the most efficient way to send messages from your language. |
