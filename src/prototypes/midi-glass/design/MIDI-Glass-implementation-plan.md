...
> **Part done, 23 September 2026.** In `src/in-box/user-tools/midi-glass/document/` and `thumbnail/`, with 55 tests in `Midi2.MidiGlass.unittests` that need no window and no device. Landed: `LayoutModel` and its validation, the serializer, `LayoutStore`, the page templates and size-derived defaults, `ThemeModel` with all nine shipped themes and measured contrast, `ThemeStore` with themes as their own shareable files, and thumbnails. Still open: auto-save with the Patchbay chip, which needs an editor to have something to save.
>
