...
void MidiComExtensionsTests::TestSendReceiveMessages()
{
    ...
    for (int i = 0; i < 50 ; i++)
    {
        std::cout << std::endl << "Testing COM extensions with single-threaded apartment round " << i+1 << ". " << std::endl << std::endl;
    ...
    std::cout << "Total STA time: " << m_totalTime << " microseconds, for " << m_countWordsSent << " words." << std::endl;
