...
#include "Enumeration.MidiGroupTerminalBlock.g.cpp"
...
namespace winrt::Windows::Devices::Midi2::Enumeration::implementation
{
    ...
    {
        winrt::hstring baseName{ midi2enum::MidiFunctionBlock::LongLabel() };
        ...
        {
            baseName = baseName + std::format(L" ({} {} to {})", midi2::MidiGroup::LongLabelPlural(), m_block.FirstGroupIndex + 1, m_block.FirstGroupIndex + m_block.GroupCount);
        }
        else
        {
            baseName = baseName + std::format(L" ({} {})", midi2::MidiGroup::LongLabel(), m_block.FirstGroupIndex + 1);
        }
    ...
    midi2enum::MidiFunctionBlock MidiGroupTerminalBlock::AsEquivalentFunctionBlock()
    {
        ...
        fb->FirstGroup(midi2::MidiGroup(m_block.FirstGroupIndex));
        ...
        switch ((midi2enum::MidiGroupTerminalBlockDirection)m_block.Direction)
        {
        case midi2enum::MidiGroupTerminalBlockDirection::Bidirectional:
            fb->Direction(midi2enum::MidiFunctionBlockDirection::Bidirectional);
            break;

        case midi2enum::MidiGroupTerminalBlockDirection::BlockInput:
            fb->Direction(midi2enum::MidiFunctionBlockDirection::BlockInput);
            break;

        case midi2enum::MidiGroupTerminalBlockDirection::BlockOutput:
            fb->Direction(midi2enum::MidiFunctionBlockDirection::BlockOutput);
            break;
        }

        switch ((midi2enum::MidiGroupTerminalBlockProtocol)m_block.Protocol)
        {
        case midi2enum::MidiGroupTerminalBlockProtocol::Unknown:
            ...
            if (m_block.GroupCount == 1)
            {
                ...
                fb->RepresentsMidi10Connection(midi2enum::MidiFunctionBlockRepresentsMidi10Connection::YesBandwidthUnrestricted);
                fb->MaxSystemExclusive8Streams(0); 
            }
            else
            {
                // We're assuming a basic MIDI 2.0 endpoint here because more than 1 group
                fb->RepresentsMidi10Connection(midi2enum::MidiFunctionBlockRepresentsMidi10Connection::Not10);
                fb->MaxSystemExclusive8Streams(1);
            }
            break;

        case midi2enum::MidiGroupTerminalBlockProtocol::Midi1Message64:
        ...
        case midi2enum::MidiGroupTerminalBlockProtocol::Midi1Message64WithJitterReduction:
            // This is MIDI 1.0, but supports 64 bit SysEx7 messages
        ...
        case midi2enum::MidiGroupTerminalBlockProtocol::Midi1Message128:
            fb->RepresentsMidi10Connection(midi2enum::MidiFunctionBlockRepresentsMidi10Connection::YesBandwidthUnrestricted);
            fb->MaxSystemExclusive8Streams(0); // MIDI 1.0 protocol, so no SysEx 8
            break;


        case midi2enum::MidiGroupTerminalBlockProtocol::Midi1Message128WithJitterReduction:
            // we don't support JR timestamps unless they are negotiated through UMP endpoint discovery/protocol negotiation
            fb->RepresentsMidi10Connection(midi2enum::MidiFunctionBlockRepresentsMidi10Connection::YesBandwidthUnrestricted);
            fb->MaxSystemExclusive8Streams(0); // MIDI 1.0 protocol, so no SysEx 8
            break;
        ...
        case midi2enum::MidiGroupTerminalBlockProtocol::Midi2:
            fb->RepresentsMidi10Connection(midi2enum::MidiFunctionBlockRepresentsMidi10Connection::Not10);
            fb->MaxSystemExclusive8Streams(1);  // guess
            break;

        case midi2enum::MidiGroupTerminalBlockProtocol::Midi2WithJitterReduction:
            fb->RepresentsMidi10Connection(midi2enum::MidiFunctionBlockRepresentsMidi10Connection::Not10);
            fb->MaxSystemExclusive8Streams(1);  // guess
            break;
        }

        fb->UIHint(midi2enum::MidiFunctionBlockUIHint::Unknown);
