...
#include "Enumeration.MidiFunctionBlock.g.cpp"
...
namespace winrt::Windows::Devices::Midi2::Enumeration::implementation
{
    winrt::hstring MidiFunctionBlock::ToString()
    {
        ...
        if (Name().empty())
        {
            baseName = baseName + internal::ResourceGetHString(IDS_DISPLAY_UNNAMED_FUNCTION_BLOCK);
        }
        else
        {
    ...
    bool MidiFunctionBlock::UpdateFromDevPropertyStruct(MidiFunctionBlockProperty prop)
    {
        ...
        m_direction = (midi2enum::MidiFunctionBlockDirection)prop.Direction;
        m_firstGroup = midi2::MidiGroup(prop.FirstGroup);
        m_numberOfGroupsSpanned = prop.NumberOfGroupsSpanned;
        m_maxSysEx8Streams = prop.MaxSysEx8Streams;
        m_uiHint = (midi2enum::MidiFunctionBlockUIHint)prop.UIHint;
        m_midi10Connection = (midi2enum::MidiFunctionBlockRepresentsMidi10Connection)prop.Midi1;
