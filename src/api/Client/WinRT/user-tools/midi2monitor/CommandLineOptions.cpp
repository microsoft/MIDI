        ...
        try
        {
            ...
            for (size_t i = 0; i < arguments.size(); i++)
            {
                ...
                if (name == L"channel" || name == L"c")
                {
                    ...
                    if (!TryParseNumberInRange(arguments[++i], 1, 16, value))
                    {
                        return MakeError(L"CommandLineErrorInvalidChannel", arguments[i]);
                    }
            ...
            if (channel.has_value() && !group.has_value())
            {
                return MakeError(L"CommandLineErrorChannelWithoutGroup", L"");
            }

            options.GroupNumber = group;
            options.ChannelNumber = channel;
        }
        catch (...)
        {
