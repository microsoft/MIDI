...
{
    ::midinetworksetup::CommandLineOptions App::s_startupOptions{};
        ...
        xaml::UnhandledExceptionEventArgs const& args)
    {
        ...
        try
        {
            TraceLoggingWrite(
                ...
                TraceLoggingWideString(L"Unhandled XAML exception. Continuing.", MIDI_NETSETUP_TRACE_MESSAGE_FIELD),
                TraceLoggingHResult(static_cast<HRESULT>(args.Exception()), MIDI_NETSETUP_TRACE_HRESULT_FIELD),
                TraceLoggingWideString(args.Message().c_str(), MIDI_NETSETUP_TRACE_ERROR_FIELD));
    ...
    void App::OnLaunched(xaml::LaunchActivatedEventArgs const& args)
    {
        ...
        try
        {
            s_startupOptions = ::midinetworksetup::CommandLineOptions::ParseProcessCommandLine();

            ::midinetworksetup::AppSettings::Current().Load();
        ...
        MIDI_NETSETUP_CATCH_AND_LOG(L"Unable to create the main window.")
    }
}
