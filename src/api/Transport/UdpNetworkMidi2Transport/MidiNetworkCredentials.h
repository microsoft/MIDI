...
// Still to be settled:
//
//  - Where the store lives and its exact ACL, and whether the service is the only writer, with
//    the Settings app handing secrets over by RPC rather than touching the file.
//  - Whether the identifier is generated by the service, which is stronger, or supplied by the
//    caller and validated, which is what IsWellFormed below currently assumes.
//  - What gates the reveal path for kind 2. Any local administrator can read the store anyway,
//    so a reveal is not a new weakness, but it should still be a deliberate act.
