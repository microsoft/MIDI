...
// we could store in place of it. The service must hold recoverable secrets. Since users reuse
// passwords, the mitigation that actually matters is not storage: it is that the Settings app
// should offer to generate the shared secret, so there is no reused password to leak.
//
// Still to be settled:
//
