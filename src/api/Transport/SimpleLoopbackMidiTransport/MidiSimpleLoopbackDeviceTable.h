...
class MidiSimpleLoopbackDeviceTable
{
private:
    std::map<std::wstring, MidiSimpleLoopbackDevice> m_devices{};


public:

    MidiSimpleLoopbackDevice* GetDevice(std::wstring associationId)
    {
    ...
    void SetDevice(std::wstring associationId, MidiSimpleLoopbackDevice device)
    {
        auto cleanId = internal::ToLowerTrimmedWStringCopy(associationId);
