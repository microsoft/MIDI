...
(
    _In_ HANDLE filter,
    _In_ ULONG pinId,
    _Out_ PKSMULTIPLE_ITEM* range,
    _Out_opt_ ULONG *rangeSize
)
{
    ...
    RETURN_HR_IF(HRESULT_FROM_WIN32(ERROR_UNSUPPORTED_TYPE), 0 == dataRanges->Count);

    *range = dataRanges.release();
    *rangeSize = dataRangesSize;

    return S_OK;
}

HRESULT
DataRangeSupportsTransport
(
    _In_ PKSMULTIPLE_ITEM dataRanges,
