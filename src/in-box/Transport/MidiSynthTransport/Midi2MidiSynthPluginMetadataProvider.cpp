...
HRESULT
CMidi2MidiSynthPluginMetadataProvider::Initialize()
{
    return S_OK;
}
...
HRESULT
CMidi2MidiSynthPluginMetadataProvider::GetMetadata(
    PTRANSPORTMETADATA metadata)
{
    ...
    // The single endpoint is created by the transport itself and cannot be added to or removed, so
    // nothing here is runtime creatable. Settings still change how it sounds and what it plays to.
    UINT32 flags{ 0 };
...
HRESULT
CMidi2MidiSynthPluginMetadataProvider::Shutdown()
{
    return S_OK;
}
