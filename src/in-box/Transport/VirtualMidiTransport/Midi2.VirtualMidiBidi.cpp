...
CMidi2VirtualMidiBidi::Shutdown()
{
    ...
    {
        if (Feature_Servicing_MIDI2VirtualDeviceClientReconnect::IsEnabled())
        {
            RETURN_IF_FAILED(TransportState::Current().GetEndpointTable()->OnClientDisconnectedKeepDeviceLink(m_endpointId));
        }
        else
        {
