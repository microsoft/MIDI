...
namespace winrt::midisettings::implementation
{
        ...
        xaml::UnhandledExceptionEventArgs const& args)
    {
        ...
        try
        {
            TraceLoggingWrite(
                ...
                TraceLoggingWideString(L"Unhandled XAML exception. Continuing.", MIDI_SETTINGS_TRACE_MESSAGE_FIELD),
                TraceLoggingHResult(static_cast<HRESULT>(args.Exception()), MIDI_SETTINGS_TRACE_HRESULT_FIELD),
                TraceLoggingWideString(args.Message().c_str(), MIDI_SETTINGS_TRACE_ERROR_FIELD));
    ...
    void App::OnLaunched(xaml::LaunchActivatedEventArgs const& args)
    {
        ...
        try
        {
            s_isElevated = ::midisettings::IsProcessElevated();

            ::midisettings::AppSettings::Current().Load();
        ...
        MIDI_SETTINGS_CATCH_AND_LOG(L"Unable to create the main window.")
    }
}
