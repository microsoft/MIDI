        ...
        try
        {
            ...
            for (size_t i = 0; i < arguments.size(); i++)
            {
                ...
                if (name == L"mode" || name == L"m")
                {
                    ...
                    auto const value = NormalizeSwitch(arguments[++i]);

                    if (value == L"bytes" || value == L"midi1")
                    {
                        mode = ScratchPadMode::Midi1Bytes;
                    }
                    else if (value == L"ump" || value == L"words")
                    {
                        mode = ScratchPadMode::UmpWords;
                    }
                    else
                    {
                        return MakeError(L"CommandLineErrorInvalidMode", arguments[i]);
                    }
            ...
            // the group only means something when MIDI 1.0 bytes are being sent
            if (group.has_value() && mode.value_or(ScratchPadMode::Midi1Bytes) != ScratchPadMode::Midi1Bytes)
            {
                return MakeError(L"CommandLineErrorGroupWithUmpMode", L"");
            }
