...
void BindingEngineTests::AnAbsoluteRangeIsDescribedInItsOwnUnits()
{
    ...
    VERIFY_IS_TRUE(engine.TryDescribeValue(0, glass::ValueAxis::X, 1.0, value, isAbsolute));
    ...
    VERIFY_IS_TRUE(engine.TryDescribeValue(0, glass::ValueAxis::X, 0.5, value, isAbsolute));
...
void BindingEngineTests::AControlThatSendsNothingDescribesNothing()
{
    ...
    VERIFY_IS_FALSE(engine.TryDescribeValue(0, glass::ValueAxis::X, 1.0, value, isAbsolute));
    VERIFY_IS_FALSE(engine.TryDescribeValue(99, glass::ValueAxis::X, 1.0, value, isAbsolute));
}

void BindingEngineTests::EachAxisIsDescribedByItsOwnRow()
{
    auto document = OneControlDocument();

    auto& control = document.Pages[0].Controls[0];

    control.Kind = glass::ControlKind::Joystick;

    auto& across = control.Messages[0];
    across.Axis = glass::ValueAxis::X;
    across.Minimum = { 0, glass::ValueScaling::Absolute };
    across.Maximum = { 127, glass::ValueScaling::Absolute };
    across.UseMidi1Protocol = true;

    auto down = across;
    down.Axis = glass::ValueAxis::Y;
    down.Number = across.Number + 1;
    down.Maximum = { 63, glass::ValueScaling::Absolute };

    control.Messages.push_back(down);
    ...
    // One value under a joystick says nothing about where the stick is, and the two axes can be
    // pointed at rows with different ends, so each has to be read through its own.
    VERIFY_IS_TRUE(engine.TryDescribeValue(0, glass::ValueAxis::X, 1.0, value, isAbsolute));
    VERIFY_ARE_EQUAL(uint32_t{ 127 }, value);
