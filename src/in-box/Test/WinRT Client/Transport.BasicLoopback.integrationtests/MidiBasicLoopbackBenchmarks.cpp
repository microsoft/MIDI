...
// Creates a transient basic loopback for a benchmark to use.
static MidiBasicLoopbackCreationResponse CreateBenchmarkLoopback(_In_ winrt::hstring const& name)
{
...
void MidiBasicLoopbackBenchmarks::BenchmarkComExtensionsSendReceive()
{
...

    {
        ...
        auto response = CreateBenchmarkLoopback(L"Benchmark Basic Loopback COM");
        ...
        auto session = MidiSession::Create(L"Basic Loopback COM Benchmark");
        ...
        auto receiveExtension = connection.as<IMidiEndpointConnectionRaw>();
        ...
        ReportBenchmarkResults(
            "Basic Loopback - 100,000 MIDI 1.0 UMPs, 10 at a time, COM extensions",
            sendStartTimestamp,
            sendEndTimestamp,
            receiveEndTimestamp,
...
// ============================================================================
void MidiBasicLoopbackBenchmarks::BenchmarkMidiMessage32SendReceive()
{
    VERIFY_IS_TRUE(MidiApi::EnsureServiceAvailable());
    VERIFY_IS_TRUE(MidiBasicLoopbackManager::IsTransportAvailable());

    auto response = CreateBenchmarkLoopback(L"Benchmark Basic Loopback Msg32");
    ...
    auto session = MidiSession::Create(L"Basic Loopback Message32 Benchmark");
    VERIFY_IS_NOT_NULL(session);
    ...
    auto eventToken = connection.MessageReceived([&](IMidiMessageReceivedEventSource const&, MidiMessageReceivedEventArgs const&)
        {
    ...
    for (uint32_t i = 0; i < BENCHMARK_MESSAGE_COUNT; i++)
    {
        ...
        auto sendResult = connection.SendSingleMessagePacket(message);
    ...
    ReportBenchmarkResults(
        "Basic Loopback - 100,000 MIDI 1.0 UMPs, MidiMessage32 + event handler",
        sendStartTimestamp,
        sendEndTimestamp,
        receiveEndTimestamp,
        BENCHMARK_MESSAGE_COUNT,
...
// a WinMM midiInOpen callback. MidiClock remains the time source.
// ============================================================================
void MidiBasicLoopbackBenchmarks::BenchmarkWinMMShortMessageSendReceive()
{
    VERIFY_IS_TRUE(MidiApi::EnsureServiceAvailable());
    VERIFY_IS_TRUE(MidiBasicLoopbackManager::IsTransportAvailable());

    auto response = CreateBenchmarkLoopback(L"Benchmark Basic Loopback WinMM");
    ...
    // The MIDI 1.0 ports are created a little after the endpoint, and their WinMM
    // port numbers are assigned asynchronously, so resolve them with retries.
    ...
    LOG_OUTPUT(WEX::Common::String().Format(
        L"WinMM source port: %u, destination port: %u", sourcePortNumber, destinationPortNumber));
    ...
    ReportBenchmarkResults(
        "Basic Loopback - 100,000 MIDI 1.0 3-byte messages, WinMM midiOutShortMsg",
        sendStartTimestamp,
        sendEndTimestamp,
        receiveEndTimestamp,
        BENCHMARK_MESSAGE_COUNT,
...
// benchmark. MidiClock remains the time source.
// ============================================================================
void MidiBasicLoopbackBenchmarks::BenchmarkWinRTMidi1SendReceive()
{
    VERIFY_IS_TRUE(MidiApi::EnsureServiceAvailable());
    VERIFY_IS_TRUE(MidiBasicLoopbackManager::IsTransportAvailable());

    auto response = CreateBenchmarkLoopback(L"Benchmark Basic Loopback WinRT MIDI 1");
    ...
    // The MIDI 1.0 ports show up a little after the endpoint does
    LOG_OUTPUT(L"Waiting for the legacy MIDI 1.0 ports to enumerate");

    winrt::hstring sourcePortId{};
    winrt::hstring destinationPortId{};

    VERIFY_IS_TRUE(TryResolveMidi1PortDeviceId(endpointId, Midi1PortFlow::MidiMessageSource, sourcePortId));
    VERIFY_IS_TRUE(TryResolveMidi1PortDeviceId(endpointId, Midi1PortFlow::MidiMessageDestination, destinationPortId));

    std::wcout << L"MIDI 1.0 source port:      " << sourcePortId.c_str() << std::endl;
    std::wcout << L"MIDI 1.0 destination port: " << destinationPortId.c_str() << std::endl;

    auto outPort = midi1::MidiOutPort::FromIdAsync(destinationPortId).get();
    VERIFY_IS_NOT_NULL(outPort);

    auto inPort = midi1::MidiInPort::FromIdAsync(sourcePortId).get();
    ...
    ReportBenchmarkResults(
        "Basic Loopback - 100,000 MIDI 1.0 messages, Windows.Devices.Midi WinRT API",
        sendStartTimestamp,
        sendEndTimestamp,
        receiveEndTimestamp,
        BENCHMARK_MESSAGE_COUNT,
