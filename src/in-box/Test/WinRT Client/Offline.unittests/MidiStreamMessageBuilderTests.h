// Copyright (c) Microsoft Corporation.
// Licensed under the MIT License
// ============================================================================
// This is part of the Windows MIDI Services App API and should be used
// in your Windows application via an official binary distribution.
// Further information: https://aka.ms/midi
// ============================================================================


#pragma once


class MidiStreamMessageBuilderTests
    : public WEX::TestClass<MidiStreamMessageBuilderTests>
{
public:

    BEGIN_TEST_CLASS(MidiStreamMessageBuilderTests)
        TEST_CLASS_PROPERTY(L"TestClassification", L"Unit")
        TEST_CLASS_PROPERTY(L"BinaryUnderTest", L"Windows.Devices.Midi2.dll")
    END_TEST_CLASS()

    TEST_METHOD(TestDeviceIdentityNotificationRoundTrip);
    TEST_METHOD(TestDeviceIdentityNotificationMasksHighBits);
    TEST_METHOD(TestDeviceIdentityNotificationRejectsOtherMessages);

private:

};
